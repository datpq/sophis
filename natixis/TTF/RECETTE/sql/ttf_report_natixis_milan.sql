...
-- TTF declaration Italian --> send to NATIXIS MILAN
spool '&2';
...
SELECT 173 || ';' || 'D' || ';' ||
    ...
    TO_CHAR(TRUNC(to_date('&1', 'YYYYMMDD'), 'MM'), 'YYYYMM') || ';' || 'NATIXIS_TTFIT_' || TO_CHAR(to_date('&1', 'YYYYMMDD'), 'YYYYMMDD') || ';' || 'NATXFRPPMAR' || ';' || 
...
WHERE STATUS_REPORT = 1 AND TTF_COUNTRY = 'IT') T
ORDER BY ID;

spool off;

UPDATE NATIXIS_TTF_AUDIT SET STATUS_REPORT = 2 WHERE STATUS_REPORT = 1 AND 'IT' = (SELECT TTF_COUNTRY FROM NATIXIS_TTF_GROUP WHERE ID = ID_GROUP);
UPDATE NATIXIS_TTFCOLLECTED_AUDIT SET STATUS_REPORT = 2 WHERE STATUS_REPORT = 1 AND TTF_COUNTRY = 'IT';
