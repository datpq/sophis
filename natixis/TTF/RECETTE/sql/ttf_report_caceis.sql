...
-- TTF declaration French --> send to CACEIS
spool '&2';
...
SELECT 173 || ';' || 'D' || ';' ||
    (NVL((SELECT SUM(TTF_AMOUNT) FROM 
        (SELECT ID_GROUP, SUM(TTF_AMOUNT) TTF_AMOUNT FROM
            ...
            FROM NATIXIS_TTF_AUDIT TA
                JOIN NATIXIS_TTF_GROUP TG ON TG.ID = TA.ID_GROUP AND TG.TTF_COUNTRY = 'FR'
            WHERE STATUS_REPORT = 1)
        GROUP BY ID_GROUP)), 0) +
    NVL((SELECT SUM(TTF_AMOUNT) FROM NATIXIS_TTFCOLLECTED_AUDIT WHERE STATUS_REPORT = 1 AND TTF_COUNTRY = 'FR'), 0)) || ';' ||
    (NVL((SELECT COUNT(*) FROM 
        (SELECT ID_GROUP, SUM(TTF_AMOUNT) TTF_AMOUNT FROM
            ...
            WHERE STATUS_REPORT = 1)
        GROUP BY ID_GROUP)), 0) +
    NVL((SELECT COUNT(*) FROM NATIXIS_TTFCOLLECTED_AUDIT WHERE STATUS_REPORT = 1 AND TTF_COUNTRY = 'FR'), 0)) ||
...
FROM
    (SELECT DISTINCT SOUSSECTION, ID_GROUP, TA.TTF_AMOUNT, TA.TAXABLE_AMOUNT, QTY_UNIT
    FROM NATIXIS_TTF_AUDIT TA
...
WHERE STATUS_REPORT = 1 AND TTF_COUNTRY = 'FR') T
ORDER BY ID;

spool off;

UPDATE NATIXIS_TTF_AUDIT SET STATUS_REPORT = 2 WHERE STATUS_REPORT = 1 AND 'FR' = (SELECT TTF_COUNTRY FROM NATIXIS_TTF_GROUP WHERE ID = ID_GROUP);
UPDATE NATIXIS_TTFCOLLECTED_AUDIT SET STATUS_REPORT = 2 WHERE STATUS_REPORT = 1 AND TTF_COUNTRY = 'FR';
