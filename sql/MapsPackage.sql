...
AS
	...
	BEGIN
		...
		loop
			...
			resultStr := GENERATE_INSERT(pTableRow.TABLE_NAME, '1 = 1', false, true, pBackupSuffix);
		end loop;
		close curTables;
	END;
    ...
    BEGIN
		...
		if p_simpleInsert then
			curInsQuery := EF.FORMAT('SELECT ''INSERT INTO {1}({2}) VALUES('' || {3} || '');'' INSERTLINE FROM {1} T{6} WHERE {4} {5}', p_tableName, fieldList, fieldValueListSql, p_whereClause, temp, curInsQueryJoin);
		else
	...
	AS
		CURSOR curTables IS
			...
			WHERE M.BK_TABLE_NAME LIKE (p_TableNamePrefix || '%') AND M.BK_TABLE_NAME LIKE ('%' || p_TableNameSuffix);
