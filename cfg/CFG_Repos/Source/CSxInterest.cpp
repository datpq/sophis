...
** Methods
*/
double CSxInterest::GetSLRoundedInterest(const CSRLoanAndRepo* loanAndRepo, double spreadHT, long date, double quantity, eDealDirection dealDirection, double &interestAmountHT)
{
	...
	if (loanAndRepoClone)
	{
		MESS(Log::debug, "Update Margin/Rate with spread " << spreadHT);
		if (loanAndRepoClone->GetCollateralCode())
			loanAndRepoClone->SetMarginOnCollateral(spreadHT);	
		else
			loanAndRepoClone->SetRateOnCollateral(spreadHT);

		double collateralInterestAtExpiry = loanAndRepoClone->GetCollateralInterestAtExpiry(date);
		double quotity = loanAndRepoClone->GetQuotity(CSRLoanAndRepo::qtInCommission);
		interestAmountHT = collateralInterestAtExpiry*quotity;
		MESS(Log::debug, "Calculate Interest Amount " << interestAmountHT << " with collateral at expiry " << collateralInterestAtExpiry << " and quotity " << quotity);
