            ...
            SELECT DISTINCT PH.GET_SLEEVE_ID(HP.OPCVM) SLEEVE_ID
                FROM JOIN_POSITION_HISTOMVTS HP
                    ...
                    JOIN TITRES T ON T.SICOVAM = HP.SICOVAM
                        AND T.AFFECTATION IN(SELECT IDENT FROM AFFECTATION WHERE LIBELLE IN ('EQUITY', 'ETF', 'REIT'))
    ...
    BEGIN
        ...
        LOOP
            ...
            SELECT ccy CURRENCY,
                ...
                CASE WHEN HP.DATENEG = p_Date AND T.AFFECTATION IN (SELECT IDENT FROM AFFECTATION WHERE LIBELLE IN ('EQUITY', 'ETF', 'REIT')) AND HP.TYPE = 1 THEN 1 ELSE 0 END OK,
                CASE WHEN ((T.TYPE = 'E' AND UPPER(DEVISE_TO_STR(T.MARCHE)) = ccy) OR (T.LIBELLE = 'Cash for currency ''' || ccy || '''')) THEN HP.QUANTITE WHEN HP.DATENEG = p_Date AND T.AFFECTATION IN (SELECT IDENT FROM AFFECTATION WHERE LIBELLE IN ('EQUITY', 'ETF', 'REIT')) AND HP.TYPE = 1
