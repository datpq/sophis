...
{
    public class MedioGeneralCommitmentAssetExposureCCYColumn : sophis.portfolio.CSMCachedPortfolioColumn
    {
        public string GetColumnName()
        {
            //return "General Commitment - Underlyings";
            return "Commitment - Underlyings";
        }

        public MedioGeneralCommitmentAssetExposureCCYColumn()
        {
                ...
                for (int idx = 0; idx < CSMCurrency.GetCurrencyCount(); idx++)
                {
                    ...
                    if (nCCY != nEURCCY)
                    {
                        using (CMString sCCY = new CMString())
                        {
                            ...
                            CSMPortfolioColumn OtherColumn = CSMPortfolioColumn.GetCSRPortfolioColumn(MedioCustomParams.OtherColumn + Utils.GetCCYName(sCCY));
                            if (OtherColumn != null)
                            {
                                ...
                                OtherColumn.GetPortfolioCell(key.ActivePortfolioCode(), key.PortfolioCode(), key.Extraction(), ref value, style, true);
                        ...
                        if (nCCY != nEURCCY)
                        {
                            using (CMString sCCY = new CMString())
                            {
                                ...
                                if (OtherColumn != null)
                                {
                                    ...
                                    OtherColumn.GetPositionCell(key.Position(), key.ActivePortfolioCode(), key.PortfolioCode(), key.Extraction(), key.UnderlyingCode(), key.InstrumentCode(), ref value, style, true);
                                    cellValue.doubleValue += value.doubleValue / fx;
                                }
                            }
                        }
