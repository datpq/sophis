...
{
    class SwapRecLegDirtyPrice : CSMPortfolioColumn
    {
        public override void GetPositionCell(CSMPosition position, int activePortfolioCode, int portfolioCode, CSMExtraction extraction, int underlyingCode, int instrumentCode, ref SSMCellValue cellValue, SSMCellStyle cellStyle, bool onlyTheValue)
        {
            ...
            try
            {
                ...
                if (swap != null)
                {
                    ...
                    {
                        cellVal = 100.0 * swapPricer.GetLegTheoreticalValue(swap, CSMMarketDataOverloader.GetCurrentMarketData(), 0);
                    }
            ...
            {
                sophis.utils.CSMLog.Write("SwapRecLegDirtyPrice", "GetPositionCell", CSMLog.eMVerbosity.M_error, "Exception Caught :" + ex.Message);
            }
        }
