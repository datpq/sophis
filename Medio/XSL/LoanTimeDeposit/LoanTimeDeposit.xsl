      ...
      public string getIntAmt(string refcon, string sicovam)
      {
            ...
            try
            {
                ...
                if (instrument != null)
                {
                    ...
                    MessageBox.Show(" type " + instrument.GetType().ToString());
                    if (debtInstrument != null)
                    {
                        int endDate = debtInstrument.GetExpiry() - 1;
                        MessageBox.Show("debt start = " + startDate + " end = " + endDate);
                        result = debtInstrument.GetAccruedCoupon(startDate, endDate);
                    }
                    else
                    {
                        int endDate = instrument.GetExpiry() - 1;
                        MessageBox.Show("instrument start = " + startDate + " end = " + endDate);
...
<!-- -->
  ...
  <xsl:template name="ope_typ" match="*">
    <xsl:variable name="model" select="/ns0:sph_otc_message/ns0:instrument/ns5:bond/ns5:pricing/ns5:model"/>
    <xsl:choose>
        <xsl:when test="contains($model, 'Loan')">
