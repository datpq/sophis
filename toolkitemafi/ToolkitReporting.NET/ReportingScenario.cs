...
namespace Eff
{
    namespace ToolkitReporting.NET
    {
        ...
        /// </summary>
        public class ReportingScenario : CSMScenario
        {
            ...
            {
                //Application.Run(CompositionRoot.Resolve<ReportingMainForm>());
                var frm = CompositionRoot.Resolve<ReportingMainForm>();
                //frm.ShowDialog();
                sophis.xaml.XSRWinFormsAdapter<ReportingMainForm> adapter = sophis.xaml.XSRWinFormsAdapter<ReportingMainForm>.GetUniqueDialog(
                    new WindowKey(12976, 0, 0), MainClass.Caption, () => frm, true);
                frm.Adapter = adapter;
